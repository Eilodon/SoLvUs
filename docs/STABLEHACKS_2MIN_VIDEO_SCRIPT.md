...
## 0:15 - 0:35
Narration:
"The key change for StableHacks is the permissioned control plane. Every mint now requires an institution profile, KYB reference, Travel Rule reference, KYT score, and a short-lived compliance permit."
...
Screen:
- Click `Mint With Phantom` once or run the smoke script phase-one output.
...
## 1:20 - 1:45
Narration:
"For an approved operator, Solvus prepares a partially signed transaction, Phantom signs the operator leg, and the mint settles on Solana. The permit is then consumed and the institution caps update."
...
## 1:45 - 2:00
Narration:
"So the product is not just a stablecoin contract. It is a permissioned BTC-backed issuance vault with compliance gating, operator controls, audit visibility, and Solana execution."
